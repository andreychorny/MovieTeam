...
//    @Query("SELECT r FROM Rating r where r.user_id = :id ORDER BY r.rating DESC")
//    List<Rating> findRatingsOfUserByIdOrderByRatingDESC(@Param("id") Long id);

    @Query("SELECT r,m FROM  Rating r JOIN Movie m ON m.id=r.movie_id WHERE r.user_id = :id ORDER BY r.rating DESC")
    List<Object[]> findRatingsAndMoviesOfUserByIdOrderByRatingDESC(@Param("id") Long id);
